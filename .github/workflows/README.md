...
- `release.yml`：正式发布入口第一版。当前提供手动 `workflow_dispatch` 的主仓库 release assemble 骨架，支持多个后端 native Actions artifact 聚合，支持从同一个历史主仓库 Release 复用多个后端 native asset，创建 draft Release、上传资产并远端回读校验；尚不支持 tag-only 全自动链路、后端来源自动解析、Web/Desktop/App 真实打包或 publish。
