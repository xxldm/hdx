...
- Release：正式 tag-only 预览发布已由 `v0.0.0-preview.5` 验证 publish、后端 resolver、主仓库 assemble 和 Full Linux AppImage sidecar/API smoke；公开端检查 run `27600342351` 已验证不再上传临时 Actions artifact。`v0.0.0-preview.6` 已因后端 `core-service` native 长时间停滞取消，临时 artifacts 已清理。仍未完成失败清理、stable 正式发布和完整安装包矩阵验证；App 当前暂不进入发布闭环。
