...
- `.github/workflows/release.yml` 已提供正式发布入口第一版：手动接收后端来源 payload，支持多个后端 native Actions artifact 聚合，支持单个历史主仓库 Release asset 来源，创建并远端校验 draft Release；尚不 publish。
- Release manifest schema、校验脚本和最小 draft 复用脚本已能表达、校验并生成历史主仓库 Release asset 复用来源、backend native fingerprint 和历史后端 asset 构建来源。

正式 tag-only 发布设计已记录在 ADR 0013 和 ADR 0014。后续仍需把主仓库 tag start、后端 release resolve、历史 Release asset 多资产聚合、Web/Desktop/App 构建、正式 publish 和失败清理串联起来。
