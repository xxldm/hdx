...
> 公开边界提示：本文只保留后端数据访问的公开默认原则。后续涉及具体模块、表结构、migration、仓储实现、SQL、验证命令和例外清单时，默认维护在后端私有子模块 `services/backend/README.md` 与 `services/backend/docs/README.md`，不要继续在公开主仓库扩写实现细节。
