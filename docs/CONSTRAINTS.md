...
- GitHub Releases 产物边界已绑定：主仓库是唯一公开发布入口；后端 private CI 先编译 native，并只通过 GitHub Actions artifact 临时交接给主仓库；主仓库 Release 可以公开后端 native archive，但不得 checkout 后端私有源码或发布 JAR/WAR、`.class`、源码和构建中间产物；真实 release workflow 使用 GitHub App token，后端 Actions artifact `retention-days: 1`；后端 native 输入未变化时可按 backend native fingerprint 复用历史主仓库 Release asset；tag-only 目标发布操作手册见 `docs/RELEASE_RUNBOOK.md`；发布 manifest JSON Schema 位于 `packages/shared/contracts/release/`；详见 `docs/adr/0012-github-releases-artifact-boundary.md`、`docs/adr/0013-release-workflow-token-and-artifact-policy.md` 和 `docs/adr/0014-release-native-build-budget-and-reuse-strategy.md`。
