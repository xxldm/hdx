...
- 公开主仓库文档只保留公开摘要、跨仓库路由、交付边界、公共契约和可公开操作说明。后端模块名、服务名、Nacos Data ID、OpenAPI 快照名和 release asset 名不作为敏感内容；Nacos、Redis、PostgreSQL、RabbitMQ、对象存储等外部工具也可以作为部署准备、环境依赖或交付边界公开出现。不得公开具体内部调用链、适配器位置、公共工具模块的职责拆分和接入方式、模块间调用关系、迁移路径、诊断数据或验证流水账。公开检查脚本也不得硬编码私有实现类名、私有表名、具体 workaround 或排障词表；需要精确拦截时走私有规则文件。后端内部实现细节、基础设施适配、内部契约、诊断数据和验证流水账默认放在后端私有子模块 `services/backend/README.md` 或 `services/backend/docs/`；产品权限矩阵、数据模型、接口草案、通知调度、公开治理、审计策略和实现切片等不适合公开的详案默认放在 `internal-docs/`。主仓库需要保留入口链接，但不得复制私有细节。
