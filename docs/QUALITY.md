...
- `-Scope docs`：检查关键文档可读取、根仓库空白错误、公开文档边界、active plan 状态索引、Release manifest 契约、Desktop Release asset 打包 fixture、OpenAPI 契约和 OpenAPI/Web 类型对齐。
- `-Scope backend`：通过 `scripts/backend-verify.ps1 -AotSmoke` 检查后端子模块，覆盖空白检查、Boot 4 Jackson 兼容性、后端测试和 AOT/package smoke。
...
- `-Profile full`：在 commit 档基础上追加 Web build、后端 AOT/package smoke 和 Desktop 编译检查。
...
- 脚本不运行完整 native-image 编译。调整后端 native 构建参数、Spring AOT、native metadata、Hibernate enhance 或类初始化参数时，仍必须按 `docs/CONSTRAINTS.md` 和后端私有文档单独验证 native 编译和健康检查。
- 后端 AOT/package smoke 覆盖 Spring AOT 与打包路径，但不生成真实 native executable。Desktop Full Linux AppImage 真实运行属于 release 产物发布后的验证，不作为本地日常质量门禁。
- 脚本通过 `scripts/git-submodule-status.ps1` 检查子模块状态：优先执行 `git submodule status`。
- 公开文档边界检查由 `scripts/check-public-doc-boundary.ps1` 执行。后端模块名、服务名、Nacos Data ID、OpenAPI 快照名和 release asset 名允许公开；后端内部调用链、适配器位置、公共工具模块职责拆分、表/迁移路径、native 诊断和验证流水账不应写入公开文档或公开脚本说明。
...
- 如果修改公开文档、根脚本或 GitHub workflow，是否仍通过 `scripts/check-public-doc-boundary.ps1`；确需公开的新模块名、服务名、Nacos Data ID、OpenAPI 快照名或 release asset 名应进入 allowlist 口径，而不是绕过检查。
