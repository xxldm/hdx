...
- 如果修改了 `docs/config/nacos/` 模板，是否已同步真实 Nacos Data ID；新增项是否已通知用户确认真实值，修改或删除项是否已获得用户同意。
- 如果修改了 `.env.example` 或 `.env.symphony.example`，是否已同步对应 `.env.local` 或 `.env.symphony.local` 文件结构；新增项是否已提示用户填写真实值，修改或删除项是否已获得用户同意。
