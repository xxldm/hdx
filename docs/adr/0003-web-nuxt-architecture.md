...
- 后端地址、本机令牌等只允许进入 Nuxt server 私有 `runtimeConfig`，不得进入 public config 或浏览器状态。
...
- 后续 Web 与后端交互必须通过公开 REST API、Nuxt server 边界和 Zod schema，不允许依赖后端源码或内部实现。
