...
ADR 0006 已确认后端公开 REST API 以 OpenAPI 作为跨端契约事实源之一，并按认证入口与业务入口拆分。根仓库也已经建立 OpenAPI 快照、路径级漂移检查和关键字段级 schema 漂移检查。
...
- 生成产物落点为 `packages/shared/generated/`。当前先以无外部依赖的 PowerShell 原型从 OpenAPI 快照生成 schema TypeScript 类型，并提交生成物与漂移检查；该原型不代表最终生成器选型。
...
- 从后端 Java DTO 直接生成 TypeScript：能减少 OpenAPI 中间层，但会让前端契约耦合后端源码和内部实现，违背公开契约边界。
...
- `docs/ARCHITECTURE.md`、`packages/shared/README.md`、`packages/shared/generated/README.md` 和 OpenAPI/shared 归档计划已同步本决策。

## 验证方式

本 ADR 和当前类型生成原型通过文档质量门禁验证：
