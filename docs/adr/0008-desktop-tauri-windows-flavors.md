# ADR 0008：Desktop 采用 Tauri 与 Local/Online 双安装包
...
- Desktop 首版需要调用 Win32 API，包括自启动、通知、URL scheme 和类似壁纸软件的桌面窗口嵌入能力。
...
通用 desktop 能力必须抽象为 capability，例如自启动、通知、deep link、托盘、配置目录、导入导出。Windows 首版先实现这些 capability；后续可以补 macOS/Linux 实现。
...
- 该能力不承诺跨平台；macOS/Linux 不需要提供等价功能。
...
- `apps/desktop/` 后续可以引入 Tauri、Rust、Tauri 官方插件和 Windows API 绑定。
...
- 后续实施 Tauri 骨架时，必须补充：
  ...
  - 自启动、通知、deep link 的平台能力测试或手工复现步骤。

## 回滚条件

满足以下任一条件时，需要新增 ADR 替代本决策：

- Tauri 无法稳定满足 Windows-only wallpaper mode 或 sidecar 管理需求。
- Tauri 的打包、签名、自动更新或 WebView 兼容性无法满足首版发布。
- Local/Online 双安装包造成不可接受的用户迁移成本，需要改为运行时切换。
- 项目决定首版不再 Windows first，而要求 macOS/Linux 同步发布。
