...
- 本机数据使用：随客户端带 Desktop Full 本机后端，使用本机数据库和本机固定身份。
...
- Desktop 第一阶段需要同时覆盖 Windows 与 Linux。Windows 需要调用 Win32 API，包括类似壁纸软件的桌面窗口嵌入能力；自启动、通知、URL scheme、托盘、配置目录和在线迁移作为通用 desktop capability 在 Windows 与 Linux 上逐项落地。
...
通用 desktop 能力必须抽象为 capability，例如自启动、通知、deep link、托盘、配置目录、在线迁移。Windows 与 Linux 在第一阶段并列实现或验证这些 capability；macOS 不进入第一阶段，后续如需支持再单独决策。
...
- 用户切换模式时安装另一个 flavor。
- 首版只提供手动导入/导出数据。
- 后续如果检测到另一个 flavor 已安装，只能提示用户按 ADR 0019 的在线迁移流程处理，不得静默读取或迁移对方数据。
- 当前不再设计 `.hdxbak` 离线导入包；本机数据与服务端账号之间只通过受控在线迁移搬家，具体边界见 ADR 0019。
...
- 两套 desktop 代码：短期分离清楚，但长期会重复维护 UI、系统能力、在线迁移、窗口和托盘逻辑，违反本项目共享边界和维护成本约束。
...
- 后续实施 Tauri 骨架时，必须补充：
  ...
  - 自启动、通知、deep link、托盘、配置目录和在线迁移的 Windows/Linux 平台能力测试或手工复现步骤。
