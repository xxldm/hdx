...
HDX 需要同时支持服务端/云端部署与 Desktop all-in-one 本地模式。前者可以依赖 PostgreSQL、Redis、对象存储和消息队列等基础设施；后者应保持轻量，不能把完整服务端运维套件塞进客户端安装包。

项目已确认：

- Redis 已用于 JWT `sid` 会话撤销/黑名单，详见 `docs/adr/0005-auth-revocation-redis.md`。
- Desktop all-in-one 不运行认证中心，使用固定本机身份与本机 token，不需要登录验证码、登录限流或 JWT 撤销。
- 对象存储代码层只使用 S3-compatible 标准核心能力，默认本地/私有化候选暂定 RustFS，后续可切云端 OSS/COS/OBS/S3。
- 队列需要服务端可部署、云端可托管、本地 all-in-one 可降级，避免业务代码直接绑定某个队列 SDK。
...
Desktop all-in-one 本地模式默认不启动 Redis，也不模拟服务端反滥用能力：
...
Desktop all-in-one 本地模式的默认基础设施边界为：

- H2：业务数据、outbox、任务状态。
- RustFS sidecar：需要本地附件/文件能力时作为 S3-compatible 对象存储，绑定 `127.0.0.1`，使用本地生成凭据和本地数据目录。
- local outbox worker：替代 RabbitMQ 执行本地异步任务。

all-in-one 不内置 RabbitMQ，不内置 Redis。local outbox worker 只承诺单机语义，不承诺 RabbitMQ 的分布式路由、DLQ 运维生态或多实例协调能力。
...
- `backend-all-in-one` 后续文件能力可以启动 RustFS sidecar，但必须通过本机端口、随机/本地生成凭据和数据目录隔离。
- `backend-all-in-one` 后续异步任务通过 H2 outbox + local worker 实现，不因服务端 RabbitMQ 决策而强制引入 RabbitMQ。
- Redis 后续扩展用途时，必须同步定义 all-in-one 的 no-op、内存、H2 或 JVM 降级行为。
...
- all-in-one 内置 Redis 与 RabbitMQ：运行时一致性更高，但安装包、端口、进程管理和排障复杂度过高，不符合本地模式轻量目标。

## 验证方式

本 ADR 阶段只固定基础设施边界，不引入运行时依赖。验证方式为：

- 使用 `Get-Content -Encoding UTF8` 读取本文档、架构文档、约束文档和计划。
- 使用 `rg` 检查 RustFS、RabbitMQ、S3-compatible、outbox worker 和 all-in-one 降级语义是否可发现。
...
- all-in-one H2 outbox + local worker 的任务成功、失败重试和重启恢复测试。
...
- Desktop all-in-one 必须与服务端保持同等分布式队列或 Redis 语义，无法接受本地降级。
