...
- 后端模块名、服务名、OpenAPI 快照名和 release asset 名不作为敏感内容；代码级重命名会影响后端私有仓库、Desktop sidecar 打包、文档、OpenAPI 调试说明和质量门禁，需后续在后端私有文档中单独计划。
- 历史计划和旧 ADR 可能保留旧内部命名作为历史上下文；后续新的发布设计、用户可见安装包和产品文案应优先使用 Full。
...
- Desktop 用户可见安装包、Tauri flavor 入口和发布设计已收敛为 Full；后端模块名可以作为部署和构建标识公开出现，但产品文案优先使用 Full。
