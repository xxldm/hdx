...
- 历史计划、旧 ADR、现有代码 feature 和当前模块名中的 `Local`/`all-in-one` 保留为历史上下文与现状描述；后续新的发布设计、用户可见安装包和产品文案应优先使用 Full，并在必要时注明当前内部模块名。

发布边界：

- GitHub Releases 后续作为公开主仓库的发布入口时，不得上传后端源码、JAR/WAR 或后端构建中间产物。
- 是否公开分发后端 native 包、Desktop Full 包或私有后端二进制，需要在发布 ADR/CI 设计中单独确认。
...
- 后续如果 `apps/web`、`apps/desktop`、`apps/mobile` 作为独立公开仓库发布，也应在各自仓库补齐 Apache-2.0 `LICENSE` 与适合自身的 `NOTICE`。

## 验证方式

- 使用 `Get-Content -Encoding UTF8` 读取本 ADR、README、架构、约束和计划。
- 使用 `rg` 检查 Apache-2.0、后端私有、JAR/WAR 禁止进入公开仓库、Full 命名边界是否可发现。
...
- 第 9 步发布设计需继续明确：不做自动部署，只通过 GitHub CI 生成包并发布到 GitHub Releases。
- 第 9 步发布设计需明确后端 native executable archive 的命名、平台、sha256、版本信息和是否进入公开 release。
- 后续单独计划将用户可见 `Local`/`all-in-one` 命名收敛为 Full；代码模块是否从 `backend-all-in-one` 改名另行确认。
