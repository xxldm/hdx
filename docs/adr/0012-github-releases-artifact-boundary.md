...
- `backend-full` 表示用户可见的本地完整后端 native 包；后端模块名/服务名不敏感，但用户可见交付优先使用 release asset 名称和交付形态名称。
- 后端 native 交付、Local/Standalone/Services 安全边界和“不发布 JVM 后端包”规则后续由 ADR 0017 补充约束。
- App 不内置后端，不提供移动端 Full 包；移动端 App 第一阶段只发布 Online 客户端，第二阶段只规划白名单离线暂存。
...
`backend-services` 包内部结构由后端私有仓库维护。公开主仓库只校验 manifest、sha256、size、平台、禁止文件规则和 release manifest 记录，不在公开 ADR 中固定内部服务文件名或配置文件名。
...
- App 提供 Full 本机后端包：与 ADR 0009 冲突，移动端已确认不内置后端，不采用。
