...
当前主仓库提供最小验证 workflow `.github/workflows/check-release-app-token.yml`。该 workflow 只用于手动验证 GitHub App token 能读取后端私有仓库和公开主仓库 metadata，不读取或下载 Actions artifact，不创建 GitHub Release，不上传 asset，也不 checkout 后端私有源码。当前 workflow 使用 `HDX_RELEASE_APP_CLIENT_ID` 与 `actions/create-github-app-token@v3.2.0` 的 `client-id` 输入，避免使用已弃用的 `app-id` 输入。
...
第一版真实 release workflow 不做跨 Release 旧资产自动复用；后端 native 构建耗时和私有 Actions 额度压力明确后，后端 native 资产复用策略由 ADR 0014 替代本段限制。
...
本节只定义正式 workflow 的设计边界；实现前不得把半成品 `.github/workflows/release.yml` 放入仓库。

正式 `release.yml` 第一版采用以下触发方式：

- 后端私有仓库 native workflow 成功后，通过 GitHub App token 触发主仓库 `release.yml`，并传入 `version`、`root_ref`、`backend_source_mode=github-actions-artifact` 和 `backend_sources_json`。
- 维护者可以手动触发 `workflow_dispatch`，使用同一组输入重跑发布或执行 `backend_source_mode=historical-release-asset` 的后端 native 复用发布。
...
- workflow 默认权限使用只读；需要读取后端 artifact、读取历史 Release asset、创建 Release、上传 asset 或 publish 时，在对应 step 附近生成短期 GitHub App token。
- 读取后端私有仓库 artifact 的 token 不复用于主仓库 Release 写操作。
- 主仓库 `release.yml` 不 checkout 后端私有源码，不缓存后端私有源码，不上传后端源码相关日志、artifact 或 cache。
...
- 主仓库真实 release workflow 后续需要按本文 job 设计使用 GitHub App token 下载后端 artifact 或历史主仓库 Release asset，并使用最小权限创建、上传和发布 draft Release。
...
- 使用 `.github/workflows/check-release-app-token.yml` 验证 GitHub App token 可以读取后端私有仓库与公开主仓库 metadata。
...
- 按本文 job 设计实现主仓库真实 `release.yml`：输入校验、root context 准备、后端新 artifact/历史 Release asset 双来源解析、Web/Desktop/App 构建、release manifest 组装、draft Release、资产上传、远端校验和 publish。
