...
当前主仓库提供最小验证 workflow `.github/workflows/release-app-token-check.yml`。该 workflow 只用于手动验证 GitHub App token 能读取后端私有仓库和公开主仓库 metadata，不读取或下载 Actions artifact，不创建 GitHub Release，不上传 asset，也不 checkout 后端私有源码。
