...
当前主仓库提供最小验证 workflow `.github/workflows/release-app-token-check.yml`。该 workflow 只用于手动验证 GitHub App token 能读取后端私有仓库和公开主仓库 metadata，不读取或下载 Actions artifact，不创建 GitHub Release，不上传 asset，也不 checkout 后端私有源码。当前 workflow 使用 `HDX_RELEASE_APP_CLIENT_ID` 与 `actions/create-github-app-token@v3.2.0` 的 `client-id` 输入，避免使用已弃用的 `app-id` 输入。
...
- 使用 `.github/workflows/release-app-token-check.yml` 验证 GitHub App token 可以读取后端私有仓库与公开主仓库 metadata。
