...
- 主仓库 release start 通过 `workflow_dispatch` 触发后端 release resolve。
- 后端 release resolve 通过 `workflow_dispatch` 触发主仓库 release assemble，并传入 `backend_source_mode` 和 `backend_sources_json`。
...
- 主仓库真实 release workflow 后续需要按本文 job 设计使用 GitHub App token 触发后端 workflow、下载后端 artifact 或历史主仓库 Release asset，并使用主仓库 `GITHUB_TOKEN` 创建、上传和发布 draft Release。
...
- 主仓库真实 `release.yml` 已完成第一版 draft assemble 骨架：支持手动输入多个后端 Actions artifact 聚合，支持从同一个历史主仓库 Release 复用多个后端 native asset，完成输入校验、root context 准备、release manifest 组装、draft Release、资产上传和远端校验。后端私有仓库已完成 release resolve 第一片，可解析指定历史主仓库 Release，或在未指定时只检查最新一个合格已发布 Release，并生成 `backend_sources_json`。后续仍需补齐 tag-only start、后端 release resolve 构建分支、Web/Desktop/App 构建、publish 和失败清理。
...
- 2026-06-10：新增 `scripts/release-resolve-backend-sources.ps1` 和后端私有仓库 `.github/workflows/backend-release-resolve.yml`，提供后端来源解析第一片：从指定历史主仓库 Release，或未指定时从最新一个合格已发布 Release，生成可交给主仓库 `release.yml` 的 `backend_sources_json`；暂不自动触发 native-image 或回调主仓库 assemble。
