...
- `.github/workflows/release.yml` 第一版已支持多个后端 Actions artifact 聚合，并支持从单个历史主仓库 Release asset 生成并校验 draft Release；完整 `resolve-backend-native` 自动 fingerprint 匹配、历史 Release asset 多资产聚合和后端 release resolve 触发仍待后续实现。
...
- 2026-06-10：新增 `scripts/release-assemble-backend-assets.ps1`，支持把多个后端 Actions artifact 聚合为统一 `backend-native-manifest.json`、`release-manifest.json`、`SHA256SUMS` 和多个后端 native Release asset；历史 Release asset 多资产复用与自动 fingerprint 匹配仍未实现。
