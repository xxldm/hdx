...
- 补齐 App 资产构建、统一 publish、失败清理策略、Desktop Full 真实安装包验证和 Desktop Online 远端配置闭环。
...
- 2026-06-12：`release.yml` 接入 Desktop Full asset 构建第一片。`resolve-backend-native` job 统一输出已校验后端资产；Desktop Full Windows/Linux job 生成 `backend-build.json`。
  ...
  Desktop Rust 侧已实现复制资源、启动本机后端、健康检查、读取 `/local/session` 和退出清理的最小闭环；真实安装包/AppImage 端到端验证和 Desktop Online 远端配置闭环仍待后续实现。
- 2026-06-13：Desktop 发布包方向改为静态 Web UI + Rust BFF。该调整不改变后端 native asset 复用规则，也不改变 Desktop Full 内置后端 archive 与公开 Release asset 同源的要求。
  ...
  Desktop Online 远端地址配置和真实安装包/AppImage 端到端验证仍待后续实现。
