...
- 下一步：第 9 步发布闭环仍不自动恢复；后端内部 GraalVM/native、运行形态和 standalone 服务端单体实现记录已迁入后端私有文档。公开主仓库只保留 release 资产边界、复用策略和私有文档入口；standalone release artifact 接入和 native runtime smoke 后续单独切片，不推 preview/stable tag。
- 主要剩余风险：总纲不承载细节；具体实现和验证以对应 active plan、ADR 或 completed plan 为准。
<!-- active-plan-status:end -->

## 阅读指引

本文件只用于判断后续步骤归属和跨计划状态；不要从这里追实现细节。认证细节读认证计划，后端 native/release 读 release native 计划，Web/Desktop 产物和 Rust BFF 读 Web/Desktop 发布产物计划。
...
- [x] 1. 收口当前 Git 状态
- [x] 2. 数据库迁移策略
- [ ] 3. 认证与权限边界（进行中，公开边界见 `docs/AGENT_BRIEF.md`、`docs/ARCHITECTURE.md`、`docs/ENVIRONMENT.md` 和 `docs/adr/0005-auth-revocation-redis.md`；后端细节见 `services/backend/README.md` 与 `services/backend/docs/README.md`）
...
状态：已完成。数据库迁移策略和运行形态边界已确定；公开总纲只记录结果，后端内部实现细节以 `services/backend/README.md` 与 `services/backend/docs/README.md` 为准。详见 `docs/plans/completed/2026-06-05-database-migration-strategy.md`。

### 3. 认证与权限边界

目标：明确 JWT issuer、权限模型、Web 登录态、desktop 本机 token 和各端身份传递边界。

状态：进行中。账号密码登录、refresh/logout、服务端会话撤销、Web 登录页、真实登录链路联调、当前身份接口、错误码契约和安全链 JSON 错误已完成。剩余风险保留在本文“剩余风险”和 `docs/AGENT_BRIEF.md`，后端实现细节以 `services/backend/README.md` 与 `services/backend/docs/README.md` 为准。
...
状态：已完成基础设施边界。服务端使用 Redis、S3-compatible 对象存储和 RabbitMQ；对象存储默认本地/私有化候选 RustFS；Desktop Full 本机模式使用本机 outbox + local worker 降级，不内置 Redis/RabbitMQ。详见 ADR 0005 和 ADR 0010。
