...
- 外部任务链接/编号：不适用
- 外部任务是否为主计划来源：否
- 当前状态：第 9 步发布与环境管理已完成公开许可、后端私有边界、GitHub Releases 产物边界、release manifest schema 设计、本地 release JSON Schema 校验和样例检查、主仓库 release dry-run workflow 骨架，以及 PowerShell 7+ / `pwsh` 运行边界收口；当前等待确认真实 GitHub Release workflow、安装器签名、公证、自动更新、release notes 或版本号策略等后续小步。
...
- 第 9 步主仓库 release dry-run workflow 骨架已确认：`.github/workflows/release-dry-run.yml` 支持手动输入 `version`、`root_ref` 和 `dry_run`，只演练输入校验、指定 root ref checkout、子模块指针记录、release manifest 校验和摘要输出；不初始化私有后端子模块、不下载后端 artifact、不创建 GitHub Release、不上传 asset、不使用跨仓库凭据。
...
- 第 9 步主仓库 release dry-run workflow 骨架已执行 workflow 静态边界扫描、`scripts/release-manifest-check.ps1` 和 docs 质量门禁；详细记录见 `docs/plans/completed/2026-06-09-release-dry-run-workflow.md`。
...
- 第 9 步发布产物边界、release manifest schema、本地 JSON Schema 校验与 release dry-run workflow 骨架已定；但真实 GitHub Release workflow、后端 artifact 下载权限、真实 release artifact 上下文一致性、Release 上传、安装器签名、公证、自动更新、release notes 和版本号策略尚未实现。
