...
- 外部任务链接/编号：不适用
- 外部任务是否为主计划来源：否
- 当前状态：进行中；第 3 步认证与权限边界仍有后续风险，第 9 步部署、发布与环境管理已有正式 `release.yml` draft assemble 第一版、多后端 Actions artifact 聚合和多历史 Release asset 复用，仍缺完整 tag-only 自动链路、签名/公证/自动更新、release notes 和版本号策略。
...
- 主仓库 tag start、后端 release resolve、Web/Desktop/App 构建、publish 和失败清理的完整自动链路。
