...
- 第 9 步 draft Release 最小闭环已新增并完成本地验证：`.github/workflows/release-draft-minimal.yml` 将使用 GitHub App token 下载后端 artifact、创建主仓库 draft Release、上传后端 native archive 与最小 manifest 资产，并从远端下载回校验 size 和 sha256；当前等待 GitHub-hosted workflow 实跑。
...
- 2026-06-09：开始 draft Release 最小闭环；新增最小 Release 资产整理脚本和手动 workflow，本地已用 run `27188320676` 产出的 artifact 生成并校验候选资产，当前等待提交推送后触发 GitHub-hosted workflow。
...
- 第 9 步 draft Release 最小闭环已执行本地脚本、`actionlint` 和空白检查；详细记录见 `docs/plans/active/2026-06-09-release-draft-minimal-workflow.md`。
