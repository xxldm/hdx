...
- 外部任务链接/编号：不适用
- 外部任务是否为主计划来源：否
- 当前状态：已确认认证中心采用独立后端模块；已确认服务端与 all-in-one 的认证持久化边界；已确认 all-in-one 固定本机身份基础字段；已确认服务端认证数据使用 PostgreSQL 独立 schema；已确认认证中心最小数据模型方向；已确认自有表字段基础规则；等待继续确认表级字段清单
...
- 角色和权限继续使用稳定 `code` 作为业务标识，用户使用 `username` 作为登录名；显示名字段只用于回显，不作为判断依据。

## 待确认事项

- 用户、角色、权限、用户角色关系、角色权限关系的表级字段清单、约束和索引。
...
优先确认认证中心自有表表级字段清单：`auth_user`、`auth_role`、`auth_permission`、`auth_user_role`、`auth_role_permission` 的字段、约束和索引。
