...
- 外部任务链接/编号：不适用
- 外部任务是否为主计划来源：否
- 当前状态：已确认认证中心采用独立后端模块；已确认服务端与 all-in-one 的认证持久化边界；已确认 all-in-one 固定本机身份基础字段；已确认服务端认证数据使用 PostgreSQL 独立 schema；已确认认证中心最小数据模型方向；已确认自有表字段基础规则；已确认用户主体与登录标识分离；已确认用户主体和登录标识表字段清单；等待继续确认角色与权限表字段清单
...
- 角色、权限、用户角色关系、角色权限关系的表级字段清单、约束和索引。
...
优先确认角色与权限表字段清单：`auth_role`、`auth_permission`、`auth_user_role`、`auth_role_permission` 的字段、约束和索引。
...
- 尚未确定角色、权限和关系表字段清单，不能开始创建完整 Flyway 脚本或实体模型。
