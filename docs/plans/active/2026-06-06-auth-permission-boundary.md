...
- 外部任务链接/编号：不适用
- 外部任务是否为主计划来源：否
- 当前状态：已确认认证中心采用独立后端模块；已确认服务端与 all-in-one 的认证持久化边界；等待继续确认本机管理员身份模型
...
- all-in-one 固定本机管理员身份的数据形状、权限表达和审计策略。
...
优先确认 all-in-one 固定本机管理员身份模型：principal 名称、权限表达、是否需要本机审计字段，以及服务端模式与本机模式的当前用户接口是否统一。
...
- 尚未确定本机管理员身份模型，不能开始改造 all-in-one 当前用户注入逻辑。
