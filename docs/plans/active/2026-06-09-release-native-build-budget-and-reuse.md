...
- `git diff --check`
- 本轮正式 `release.yml` 设计收口只做文档和契约一致性验证；不运行 GitHub-hosted release workflow。

## 风险与阻塞

- 并行 services 构建降低墙钟时间，但不会降低 GitHub Actions runner 分钟总消耗，可能略增。
- `release-manifest.json` schema、校验脚本和手动最小 draft workflow 已能表达、校验、下载并重新上传历史 Release asset；ADR 0013/0014 已补齐正式 `release.yml` 设计，但完整真实 release workflow 仍未实现。
...
- `.github/workflows/release.yml` 尚不存在；后续实现时必须先按设计拆出 `validate-inputs`、`prepare-root-context`、`resolve-backend-native`、构建、组装、draft、远端校验和 publish job，避免直接复制 debug workflow 拼接成正式发布。
...
- 并行 services 构建降低墙钟时间，但不会降低 GitHub Actions runner 分钟总消耗，可能略增。
- 完整真实 `release.yml` 已有第一版设计，但尚未实现；后端 artifact 新建分支、历史 Release asset 复用分支、Web/Desktop/App 构建、正式 publish 和失败清理仍未串成可运行 workflow。
...
- 历史复用契约、校验脚本和正式 `release.yml` 设计更新由本计划后续提交记录补齐。
