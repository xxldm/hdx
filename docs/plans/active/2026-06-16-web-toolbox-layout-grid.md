...
- 当前状态：Web 首页工具箱布局已完成主体实现；首页外壳改为全窗口工具箱，右侧概览和介绍已移除。编辑态支持整卡拖动、右下角拖动缩放、非法缩放保护、组件内容切换和本地持久化；顶部操作、头像菜单和卡片动作已统一回 Nuxt UI 控件。布局模型已从自动紧凑排序改为显式保存 `row`/`column`/跨度，允许空格真实存在；旧 order-only 本地数据会迁移为坐标布局。添加组件不再作为真实 grid child 渲染，改为 hover 到空白格时显示悬浮入口并按该坐标添加。拖拽让位动画已改为手动 FLIP 兜底；拖到组件时，提交路径从“直接交换/最近空位”收口为“按拖动方向最小必要格数挤开”，更接近手机桌面顺延手感。
- 下一步：清理根仓库收尾，更新 `apps/web` 子模块指针和 active plan 索引；后续接入真实模块组件注册契约，并在恢复认证闭环前重新启用未登录跳转。
- 主要剩余风险：真实模块组件尚未接入；`auth.global.ts` 当前临时注释未登录跳转，不能带入正式认证闭环；移动端编辑态与更复杂的多卡连锁顺延路径仍可继续打磨。
...
- 已通过：`pnpm test`、`pnpm typecheck`、`pnpm lint`、`pnpm build`、`git diff --check`；空白格 hover 添加和小组件落到大组件内部非左上格子修复后已重新通过 `pnpm test`、`pnpm typecheck`、`pnpm lint`。2026-06-18 又在“最小挤开”规则接入后重新通过 `pnpm test`、`pnpm typecheck`。
- 浏览器验证：Chrome 已打开 `http://localhost:3000/`；确认头像菜单可打开并点外部关闭、编辑态可打开、整卡拖动排序可提交、右下角拖动缩放可实时改变跨行跨列，且缩放时卡片保持透明度反馈而不是变白。2026-06-18 用户确认“快捷入口”挤压与回位的手感已明显改善，当前感觉不错。
