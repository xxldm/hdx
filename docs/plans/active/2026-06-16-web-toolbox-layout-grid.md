...
- 当前状态：已按 ADR 0016 收口工作台布局、计时器预设、账号级用户偏好和日期倒计时节日数据。`docs/WORKBENCH_WIDGET_CONTRACT.md` 已明确 widget registry、layout、模块数据、模块配置和设备运行态边界；Web registry 已显式声明 timer 的账号级预设接口、设备级运行态，以及 date-countdown 的共享节日数据接口。节日管理员维护页已接入后端、Web、Desktop BFF 和 OpenAPI 契约；日期倒计时定位为人工维护的事件/节日数据，不自动维护调休或复杂日历规则。
- 下一步：继续接入新真实模块前，先按 `docs/WORKBENCH_WIDGET_CONTRACT.md` 判断模块数据归属，再决定是否新增独立后端契约；后续 todo、随手记等模块优先按独立模块 API、store/composable 和可选 `configRef` 接入，不把业务数据塞进 layout。若继续完善日期倒计时，优先处理人工维护体验、节日权限和 `holiday_key` 软删除复用策略。
- 主要剩余风险：日期倒计时当前依赖人工维护事件/节日数据，自动调休、农历推算和工作日历同步暂不进入当前范围。`holiday_key` 当前全局唯一，软删除后暂不复用。用户偏好目前按整体对象版本保存，低价值偏好冲突会在 Web 端基于服务器当前版本重试，高价值模块配置必须进入模块自己的记录级冲突模型。
<!-- active-plan-status:end -->

## 目标

把 Web 登录后的首页从运行状态面板改成用户端工具箱首页：支持网格行列与间距配置，组件可跨行跨列，真实组件可拖拽排序。布局当前以账号级后端事实源为准，端侧只保留必要草稿和运行态；后续业务模块应通过明确的组件定义接入，而不是把组件列表和布局规则散落在页面里。

## 非目标

- 不实现真实模块市场或权限化组件分发；布局同步已进入账号级后端事实源。
...
- [x] 新增首页菜单抽屉和顶栏固定菜单偏好；该偏好后续已迁入账号级用户偏好事实源。
...
- 不设计后端动态提供 Vue 组件；当前注册表是 Web 静态边界。后续如需不同用户可用组件不同，只让后端返回模块 key、权限和能力摘要。
...
- 2026-06-21：确定首页导航形态为左侧菜单抽屉加桌面宽屏顶栏固定项；当时用户置顶偏好由独立 `workbench-navigation` store 本地持久化，后续已迁入账号级用户偏好事实源。菜单里的组件项先定位/高亮首页已有 widget，后续真实页面接入时只更新 navigation registry。
...
- 2026-06-26：确认日期倒计时当前以人工维护事件/节日数据为主，暂不设计自动维护调休、农历节日推算或工作日历同步；后续 todo、随手记等真实模块按独立模块 API、store/composable 和可选 `configRef` 接入，layout 继续只保存展示偏好和必要引用。
- 2026-07-08：明确首页组件只提供可选项和默认实例；新账号默认布局只是启动建议。Todo、日程、规则状态和公开流可以提供首页组件，公开发现、候选池、空间管理、实例管理和协作事项不作为首页一级组件。
...
- 2026-06-22 至 2026-06-23：工作台布局、计时器预设、账号级用户偏好、日期倒计时和节日管理员维护页的后端持久化、OpenAPI 快照、Web、Desktop BFF 与数据访问检查均已通过当时受影响范围验证。具体后端模块、测试类、数据访问提醒和命令流水账不再在公开 active plan 展开；需要追溯后端实现细节时读后端私有文档。
...
- 日期倒计时已接入节日数据和管理员维护页；当前产品边界是人工维护事件/节日数据，自动调休、农历推算和工作日历同步暂不进入当前范围。
