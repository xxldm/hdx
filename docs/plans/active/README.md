...
| [2026-06-16-web-toolbox-layout-grid.md](2026-06-16-web-toolbox-layout-grid.md) | 修改 Web 首页工具箱布局、模块组件接入、布局持久化或首页视觉风格时读取。 | Web 首页工具箱布局已完成主体实现；编辑态支持整卡拖动排序实时预览、右下角拖动缩放实时预览、非法缩放保护、按钮兜底调整和本地持久化。已临时放开未登录跳转用于首页视觉和布局调试。 | 按用户确认决定是否提交/推送；恢复认证闭环前重新启用未登录跳转。 | 真实模块组件尚未接入；`auth.global.ts` 当前临时注释未登录跳转，不能带入正式认证闭环。 |
