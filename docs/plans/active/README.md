...
| [2026-06-16-web-toolbox-layout-grid.md](2026-06-16-web-toolbox-layout-grid.md) | 修改 Web 首页工具箱布局、模块组件接入、布局持久化或首页视觉风格时读取。 | Web 首页工具箱布局已完成主体实现；首页外壳改为全窗口工具箱，右侧概览和介绍已移除。编辑态支持整卡拖动、右下角拖动缩放、非法缩放保护、组件内容切换和本地持久化；顶部操作、头像菜单和卡片动作已统一回 Nuxt UI 控件。布局模型已从自动紧凑排序改为显式保存 `row`/`column`/跨度，允许空格真实存在；旧 order-only 本地数据会迁移为坐标布局。添加组件不再作为真实 grid child 渲染，改为 hover 到空白格时显示悬浮入口并按该坐标添加。拖拽让位动画已改为手动 FLIP 兜底；拖到组件时，提交路径从“直接交换/最近空位”收口为“按拖动方向最小必要格数挤开”，更接近手机桌面顺延手感。当前边界明确为桌面视口优先，不做手机 Web 专项适配，但保留桌面宽度触摸输入。 | 提交本轮文档边界更新；后续接入真实模块组件注册契约，并在恢复认证闭环前重新启用未登录跳转。 | 真实模块组件尚未接入；`auth.global.ts` 当前临时注释未登录跳转，不能带入正式认证闭环；桌面宽度触摸输入与更复杂的多卡连锁顺延路径仍需随着后续组件接入持续自测。 |
