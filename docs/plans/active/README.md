...
| [2026-06-09-release-native-build-budget-and-reuse.md](2026-06-09-release-native-build-budget-and-reuse.md) | 后端 native artifact、GitHub Actions release start、历史 Release asset 复用、后端 resolver 相关任务。 | `v0.0.0-preview.2` 真实 tag-only 预览发布已成功，Full Linux AppImage 真实运行暴露 Jackson 2/3 `ObjectMapper` 类型不匹配；后端已改用 Jackson 3，并把 Boot 4 Jackson 静态检查、all-in-one AOT/package smoke 固化到门禁。 | 推送本轮门禁补强和子模块指针后，发布 `v0.0.0-preview.3`，再在本机 Ubuntu WSL 复测 Full Linux AppImage 的真实 sidecar 启动、`/local/session` 和工作台数据加载；WSL/AppImage 真实 smoke 属于 release 后验证。 | `v0.0.0-preview.1` 失败 draft 已保留用于排障，`v0.0.0-preview.2` 是测试 prerelease 且 Full Linux AppImage sidecar 已确认不可用；后端修复尚未经过新版真实 Linux native/AppImage 产物复测。Windows services 包、旧 workflow 复现和很旧 tag 的 workflow 入口仍需后续设计或验证。App 当前暂不进入发布闭环。 |
