...
- Flyway 新依赖已在本轮 Maven 验证中解析成功；后续新环境首次构建仍可能需要联网下载依赖。
- Flyway 与 Spring Boot 4 的测试、AOT/package 入口和后续真实 PostgreSQL service profile 已验证通过；完整 native-image 编译仍见“剩余风险”。
...
- 2026-06-05：`mvn validate`、`mvn test` 和 native profile 轻量包构建已通过；随后完成提交、推送和最终状态收口。
- 2026-06-05：已在 `services/backend` 创建并推送提交 `9090455 功能：引入 Flyway 数据库迁移`；根仓库随后提交子模块指针和计划更新。
- 2026-06-07：复核本计划任务清单、验证结果、后端迁移脚本和总纲状态后，确认数据库迁移策略已完成并移动到 `docs/plans/completed/`。
- 2026-06-07：后续认证与 Nacos 联调已覆盖真实 PostgreSQL service profile 启动：auth/core/gateway 均可读取 Nacos Data ID 并启动，Flyway 确认 `auth` 与 `public` schema 迁移版本有效。
...
- 后续 `backend-auth-service` 真实 PostgreSQL service profile `/actuator/health` 返回 `UP`，验证 `auth` schema 迁移脚本可执行；后续真实 Nacos service profile 联调确认 core/auth 连接 PostgreSQL 并完成 Flyway 校验。

## 剩余风险

- 真实 PostgreSQL service profile 启动和健康检查已由后续认证/Nacos 联调覆盖；本计划保留该交叉验证记录，事实源见 `docs/plans/active/2026-06-06-auth-permission-boundary.md`。
- 尚未在引入 Flyway 后单独运行完整 native-image 编译；本轮只验证到 Spring AOT 与 native profile 跳过 native-image 的包构建路径。
