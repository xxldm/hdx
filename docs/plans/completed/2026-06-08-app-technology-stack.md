...
- 本轮不规划移动端 all-in-one、本机后端服务或完整离线业务引擎。
...
- 明确不规划移动端 all-in-one、本机后端服务或完整离线业务引擎。
- 相关文档之间没有互相矛盾的旧状态。

## 验证方式

- `Get-Content -Encoding UTF8` 读取新增和更新的中文文档。
- `rg -n "App 当前阶段仍不绑定|App 技术栈|移动端 all-in-one|第三阶段" docs apps/mobile`
...
- 2026-06-08：确认移动端只保留首版 Online only 与第二阶段离线缓存/离线草稿，不规划移动端 all-in-one、本机 HTTP 后端服务或完整离线业务引擎。
...
- 已执行 `rg -n "App 当前阶段仍不绑定|技术栈.*尚未决定|第三阶段|完整离线版|移动端 all-in-one|本机 HTTP 后端|backend-all-in-one" docs apps/mobile`：未发现 App 技术栈未决策的旧状态；命中项均为明确不规划移动端 all-in-one/完整离线业务引擎的新边界，或 Desktop/后端历史记录。
